...
def generate_page_html(title, page_heading, current_page, content, subtitle="", page_description="", custom_css="", custom_js="", extra_head=""):
    ...
    # Find active faction based on current page
    active_suit = "default"
    ...
    colors = {"bird": "#67c0c7", "fox": "#e6372d", "rabbit": "#f7eb5b", "mouse": "#f29057", "default": "#d4a76a"}
    main_color = colors.get(active_suit, colors["default"])

    # 1. Main Navigation with Data Attributes for JS
    nav_links = [("index", "Leaderboard", "bird"), ("matches", "Top Tables", "fox"), 
                 ("trends", "Player's Journey", "rabbit"), ("about", "Codex", "mouse")]
    nav_html = ""
    for base, label, faction in nav_links:
        if base == "about": target = "about.html"
        else: target = f"{base}_lh01.html" if is_archive else f"{base}.html"
        active = "active" if current_page.startswith(base) else ""
        # Adding data-faction for the hover effect
        nav_html += f'<a href="{target}" class="{active}" data-faction="{faction}">{label}</a>'
    ...
    if current_page != "about.html":
        ...
        sub_nav_html = f'<div class="season-selector"><span class="season-label">Select Season</span>{btns}</div>'
    ...
    base_css = f"""
        ...
        .icon-highlight {{ 
            ...
            filter: drop-shadow(0 15px 25px rgba(0,0,0,0.9)) !important;
            z-index: 100;
        }}
        .icon-highlight.left {{ transform: translateY(-20px) scale(1.2) rotate(-8deg); }}
        .icon-highlight.right {{ transform: translateY(-20px) scale(1.2) rotate(8deg); }}

        .site-title {{ color: #eee; font-size: 3.5em; margin: 0; letter-spacing: 5px; text-transform: uppercase; font-weight: 900; }}
        ...
        .page-intro h3 {{ color: {main_color}; font-size: 1.1em; font-weight: bold; text-transform: uppercase; border-bottom: 1px solid {main_color}; padding-bottom: 4px; display: inline-block; }}
...
<body>
    <div class="container">
        <div class="banner-container">
            ...
            <div class="banner-icons right">
                ...
                <img src="assets/icons/mouse_2.webp" id="icon-mouse">
            </div>
        </div>

        <nav id="main-nav">{nav_html}</nav>
        {sub_nav_html}
    ...
    <script>
        // Synchronize Nav Hover with Banner Icons
        const navLinks = document.querySelectorAll('#main-nav a');
        
        navLinks.forEach(link => {{
            link.addEventListener('mouseenter', () => {{
                const faction = link.getAttribute('data-faction');
                const icon = document.getElementById('icon-' + faction);
                if (icon) {{
                    const sideClass = icon.closest('.banner-icons').classList.contains('left') ? 'left' : 'right';
                    icon.classList.add('icon-highlight', sideClass);
                }}
            }});
            ...
            link.addEventListener('mouseleave', () => {{
                ...
                const icon = document.getElementById('icon-' + faction);
                if (icon) {{
                    icon.classList.remove('icon-highlight', 'left', 'right');
                }}
            }});
        }});
    </script>
...
def build_about_page(filename, title, heading):
    ...
    <style>
        ...
        .tier-item img {{ height: 90px; width: auto; margin-right: 15px; }}
