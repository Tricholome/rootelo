"""frog.py — Dynamic player homelands simulation over a season with tier classification.
...
def compass(G, roster, eligible, ever_used_names):
  ...
  for i, c in enumerate(comms):
    if (
        i not in name_of
        and len(c) >= MIN_TRIBE_CREATION_SIZE
        and free
        and is_clearly_apart(G, roster, c)
    ):
...
def tribe_core_scores(G, members):
  """Weighted-degree centrality within own tribe subgraph."""
  n = len(members)
  if n < 2:
    return {}
...
def classify_tribe_tiers(scores):
  ...
  if len(values) >= 2:
    ...
    pillar_cut = q[PILLAR_PERCENTILE - 1]
    satellite_cut = q[SATELLITE_PERCENTILE - 1]
  else:
    pillar_cut = satellite_cut = values[0]

  pillars = [p for p, v in items if v >= pillar_cut]
  if len(pillars) < PILLAR_MIN:
    pillars = [p for p, _ in items[:PILLAR_MIN]]
  elif len(pillars) > PILLAR_MAX:
    pillars = [p for p, _ in items[:PILLAR_MAX]]
  ...
  satellites = satellites[:SATELLITE_MAX]
...
for m in matches:
  date_str = str(
      m.get("Date") or m.get("date") or m.get("date_closed") or ""
  )[:10]
  players = list(
...
for d in sorted_dates:
  ...
  tribe_layout = {}  # Dynamic core/satellite classification for graph visuals
  ...
  for t in active_tribes:
    ...
    # Classify tribe members into Pillars and Satellites
    pillars, satellites = classify_tribe_tiers(t_scores)
    tribe_layout[t] = {
  ...
  snapshots[d] = {
      ...
      "summary": summary,
      "tribe_layout": tribe_layout,  # Exported layout structure
...
if (Path(TEMPLATE_DIR) / TEMPLATE_FILE).exists():
  ...
  print(
      f"✅ File {OUTPUT_FILE} successfully generated ({len(snapshots)} dates)."
  )
